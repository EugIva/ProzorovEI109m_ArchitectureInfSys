...
def create_recipient(recipient: RecipientCreate, db=Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Recipient creating failed. Mayby because login not unique")


@ app.put("/update_recipient")
def update_recipient(recipient_id: int, recipient: RecipientUpdate, db=Depends(get_db), security: HTTPAuthorizationCredentials = Depends(security)):
    try:
